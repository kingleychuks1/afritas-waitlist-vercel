...
// Same-origin on Vercel: leave blank. Override with VITE_API_URL only if the
...
function WaitlistForm({ id }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setStatus("ok");
      setMessage("You're on the list! Check your inbox for confirmation soon.");
      setEmail("");
    } catch (err) {
      setStatus("err");
  ...
  return (
    <form className="hero-form-wrap" onSubmit={handleSubmit} id={id}>
      ...
      <p className="form-note">Free to join. No spam, unsubscribe anytime.</p>
      {message ? (
        <p className={`form-message ${status === "ok" ? "ok" : "err"}`}>{message}</p>
      ) : null}
    </form>
  );
}
